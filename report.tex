...
\textbf{XGBoost DIO Optimization on Deep Features:} Achieved \textbf{83.0\% accuracy} on optimization subset (2K train, 500 test) with \textbf{598/2048 features (70.8\% reduction)}, outperforming baseline (80.8\%) by +2.2\% while reducing inference cost by ~3.4×. Full dataset baseline (50K samples) achieved 85\% accuracy, demonstrating model selection effectiveness. Optimization completed in 3.6 hours (215.98 minutes) on stratified subset, demonstrating scalability to high-dimensional spaces.

\textbf{Cross-Domain Methodological Insights:}
\begin{enumerate}
    \item \textbf{Consistent Value Delivery:} DIO improved accuracy-complexity trade-offs in both 30-D medical data (up to +2.18\% accuracy, 80-87\% feature reduction) and 2048-D image features (single-run +2.2\% but failed statistical validation, 70.8\% reduction).
...
\begin{itemize}
    ...
    \item \textbf{Edge Deployment:} Reduced feature dimensionality (2048→598) enables deployment on resource-constrained IoT devices, smartphones, and embedded systems
