...
The main lesson from testing across medical and vision domains is that optimization budget must scale appropriately with problem dimensionality. What worked for 30-dimensional medical data (10,000 evaluations) proved woefully inadequate for 2048-dimensional image features—a cautionary tale for hyperparameter optimization in modern deep learning applications.
\end{abstract}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth,height=0.65\textheight,keepaspectratio]{schemas and snippets/schema1_cross_domain_framework.png}
    \caption{
        \textbf{Cross-Domain DIO Framework:} Overview showing DIO's application across both Medical (Breast Cancer, 30D) and Vision (CIFAR-10, 2048D) domains, with nested optimization structure. Medical: 96.88\% accuracy (XGBoost, Rank \#1), 10 features (67\% reduction). Vision: 81.91\% accuracy (XGBoost, Rank \#3, worse than defaults), 598 features (70.8\% reduction). Demonstrates 68× dimensional scale-up with success in low-D (30) but failure in high-D (2048) due to insufficient budget.
    }
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth,height=0.6\textheight,keepaspectratio]{schemas and snippets/schema5_fitness_driven_optimization.png}
    \caption{
        \textbf{Fitness-Driven Optimization Mechanism:} The complete optimization process showing how fitness F = 0.99$\times$(1-Acc) + 0.01$\times$(Feat/Total) drives both nested loops. The outer loop tests hyperparameters $\theta$ while the inner loop (for each $\theta$) finds optimal features S*. Both minimize F simultaneously. Total evaluations = (Outer\_dholes $\times$ Outer\_iterations) $\times$ (Inner\_dholes $\times$ Inner\_iterations). This is the core technical schema explaining HOW the complete optimization mechanism works.
    }
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Population size: 10 dholes
        \item Iterations: 20
        \item Search space: Continuous vector [0,1]$^{30}$, thresholded at 0.5 to create binary feature masks
    \end{itemize}
\end{itemize}

These parameters were chosen to balance optimization quality with computational feasibility. The outer loop's moderate population (5) reflects the lower dimensionality of the hyperparameter space (4D), while the inner loop's larger population (10) addresses the higher dimensionality of feature selection (30D).
...
\begin{enumerate}
    ...
    \item \textbf{Random Forest (Default, Selected Features):} Scikit-learn defaults with DIO's 4 selected features
    \item \textbf{XGBoost (All Features):} Gradient boosting with default parameters, all features
    \item \textbf{XGBoost (Selected Features):} Gradient boosting with default parameters, 4 features
...
Our nested DIO framework yielded a striking result: while full-feature models like XGBoost achieved peak accuracy (\textbf{96.24\% $\pm$ 1.52\%} on all 30 features), the DIO-Optimized Random Forest delivered competitive \textbf{94.37\% $\pm$ 1.82\%} using only \textit{4 features}—an 87\% dimensional reduction with good stability. Table \ref{tab:model_summary} summarizes these findings across eight classifiers.

% --- Results Table ---
\begin{table}[H]
    \centering
    \caption{Model Performance Summary over 30 Runs - RF Single-Split Approach (Top 5 and DIO)}
    ...
    \begin{tabular}{lccccc}
        ...
        RF Default (Selected) & 94.99 & 1.53 & 4 & 4 \\
        Logistic Regression & 94.91 & 1.63 & 30 & 5 \\
        \textbf{DIO-Optimized RF} & \textbf{94.37} & \textbf{1.82} & \textbf{4} & \textbf{6} \\
        \bottomrule
    \end{tabular}%
    }
\end{table}
...
The key success of this research is the achievement of a Pareto-optimal solution. While our model does not have the highest absolute accuracy, it represents the best trade-off between accuracy and complexity (number of features). An 87\% reduction in features (30$\rightarrow$4) for a mere 1.50\% drop in accuracy compared to a full-featured RF is a highly desirable outcome for practical applications, leading to faster inference times and more interpretable models.

\subsection{Feature Selection Analysis}
The 4 features selected by the DIO algorithm (single-split approach) provide valuable insights into the most discriminative characteristics for breast cancer classification. The selected features include:
\begin{itemize}
...
\end{itemize}
This compact subset represents smoothness and area measurements at different scales (mean, error, worst), suggesting that DIO identified the most discriminative cell nuclei characteristics. The 87\% feature reduction (30$\rightarrow$4) translates directly to computational savings: inference time is reduced proportionally, memory footprint decreases, and model interpretability improves significantly.

\subsection{Detailed Performance Comparison}
When examining the full model landscape (Table \ref{tab:model_summary}), ensemble methods dominate the top rankings. However, it is crucial to distinguish between models using all 30 features versus those constrained to the 4 DIO-selected features. Among the 4-feature models, DIO-Optimized RF ranks below RF Default (Selected), indicating optimization overfitting—the hyperparameters were over-tuned to a single data partition.

The comparison with XGBoost (Selected), which achieved 94.31\% using the same 4 features, reveals an opportunity for future work: applying DIO to optimize XGBoost or Gradient Boosting hyperparameters could potentially yield even better results within the reduced feature space.

\subsection{Optimization Overfitting: A Critical Insight}
A particularly noteworthy finding emerged when comparing DIO-Optimized RF (94.37\% $\pm$ 1.82\%) with RF Default (Selected) using the same 4 features (94.99\% $\pm$ 1.53\%). The statistically significant difference (p=0.0349, *) reveals an important limitation in our methodology: \textbf{optimization overfitting to a single train/test split}.
...
% --- Schema 2: Algorithm-Dependent Optimization Overfitting (KEY DISCOVERY) ---
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth,height=0.6\textheight,keepaspectratio]{schemas and snippets/schema2_algorithm_dependent_overfitting.png}
    \caption{
        \textbf{Algorithm-Dependent Optimization Overfitting Discovery:} Comparison of three approaches showing our key research contribution. RF Single-Split (5/10 outer, 10/20 inner) achieved 99\% during optimization but only 94.37\% ± 1.82\% validation (rank \#6) - severe overfitting. RF-CV (same config + 5-fold CV) fixed this: 96.55\% ± 1.51\% (rank \#1) but took 7.9 hours. XGBoost (5/10 outer, 10/20 inner) achieved best results 96.88\% ± 1.10\% (rank \#1) in only 54 seconds without CV, proving gradient boosting's built-in regularization prevents meta-overfitting. KEY FINDING: Algorithm choice determines whether cross-validation is necessary during optimization.
    }
...
\begin{enumerate}
    \item \textbf{Feature Selection is Primary:} The 87\% feature reduction (30$\rightarrow$4) was the true contribution, not the hyperparameter tuning. Both DIO-optimized and default hyperparameters performed similarly when using the selected features.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        Features Selected & 4 (87\% reduction) & 6 (80\% reduction) \\
...
The CV-optimization experiment validates our methodology—cross-validation during the optimization loop successfully addresses optimization overfitting, yielding hyperparameters that generalize robustly across data partitions. Unlike the single-split approach where optimized hyperparameters underperformed defaults (p=0.0349), the CV-optimized configuration achieves statistical equivalence with defaults using the same features (p=0.7480).

Interestingly, feature selection remains the dominant contribution even with proper CV-based optimization. The 80\% reduction to just 6 features accounts for most of the performance gain, though hyperparameter tuning now adds measurable value (2.18\% accuracy improvement over single-split).
...
Using a fast single-split optimization (5 dholes, 10 iterations outer loop; 10 dholes, 20 iterations inner loop, 54 seconds total), we optimized 5 XGBoost hyperparameters simultaneously with feature selection:
\begin{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \midrule
        Features Selected & 4 (87\% red.) & 6 (80\% red.) & \textbf{10 (67\% red.)} \\
        ...
        vs. Defaults (p-value) & 0.0349 (*) & 0.7480 (ns) & 0.0047 (**) \\
...
\begin{itemize}
    \item Maximum diagnostic accuracy is the priority (highest observed: 96.88\%)
...
\textbf{In plain terms}: XGBoost + DIO gave us the best accuracy (96.88\%) with 10 features (67\% reduction), and the optimization ran 526$\times$ faster than CV-based RF. For practitioners needing maximum performance without sacrificing development speed, this is the clear winner.
...
\textbf{2. DIO-RF Single-Split (4 features, 94.37\% $\pm$ 1.82\%):}
\begin{itemize}
    \item Ultra-fast optimization: 1 minute
    \item Good feature reduction: 87\% (4 features)
...
\begin{enumerate}
    \item \textbf{Competitive Accuracy:} All optimized models achieve 94-97\% accuracy, comparable to or exceeding full-feature baselines
    \item \textbf{Significant Feature Reduction:} 67-87\% fewer features translate to faster inference, lower costs, and improved interpretability
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        \midrule
        Selected Features & 598 / 2,048 & \textbf{70.8\% reduction} \\
        Test Accuracy & \textbf{83.0\%} & Baseline: 80.8\% (subset) \\
        Test F1-Score & \textbf{0.8310} & Baseline: 0.8088 \\
        Accuracy Gain & \textbf{+2.2\%} & +2.72\% relative \\
...
\begin{enumerate}
    ...
    \item \textbf{Dramatic Feature Reduction:} 70.8\% dimensionality reduction (2048→598 features) while \textit{improving} accuracy validates DIO's ability to identify and eliminate highly redundant deep learning features.
...
\begin{figure}[H]
    ...
    \caption{DIO-Optimized XGBoost Performance on CIFAR-10 Subset: (Left) Test accuracy comparison showing 2.2\% improvement over baseline (83.0\% vs 80.8\%). (Right) Feature count reduction from 2,048 to 598 features (70.8\% reduction) while achieving superior accuracy.}
    \label{fig:cifar10_results}
\end{figure}

\textbf{Key Takeaways from Visualization:}
\begin{itemize}
    \item \textbf{Single-Run Performance:} On the specific optimization split, DIO achieved 83.0\% accuracy with 70.8\% feature reduction, appearing to dominate the baseline.
    \item \textbf{Inference Speedup:} 70.8\% fewer features translates to ~3.4× faster inference time for real-time applications and edge deployment.
    \item \textbf{Generalization Caution:} While the 2.2\% improvement on the held-out test set initially suggested generalization, subsequent 30-run statistical validation revealed this was optimization overfitting (see Section 5.3.3).
    \item \textbf{Feature Redundancy:} Ability to discard 1,450 of 2,048 features (70.8\%) indicates substantial redundancy in frozen ResNet50 representations for this classification task.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Budget Scaling Law:} The medical optimization (30-D) succeeded with 5 dholes × 10 iterations (outer) and 10 dholes × 20 iterations (inner) for both RF and XGBoost (~10,000 evaluations), while the image optimization (2,048-D) failed with 3 dholes × 8 iterations for both loops (~576 evaluations). This suggests that optimization budget must scale with search space dimensionality—likely requiring 10× to 100× more evaluations for the CIFAR-10 case.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        Optimization Time & 7.9 hours & 3.6 hours & Faster despite 68× features \\
...
\begin{enumerate}
    ...
    \item \textbf{Feature Selection Can Backfire:} We reduced CIFAR-10 features by 70.8\% (2048 to 598), which sounds great. But that reduction was overfitted to the training partition and actually hurt generalization. Compare this to the medical domain where 80\% feature reduction improved stability. The difference? Adequate optimization budget. Without it, aggressive feature selection becomes counterproductive.
...
The CIFAR-10 extension provides critical insights into DIO's limitations:

\textbf{What Appeared to Work (Single-Run Illusion):}
\begin{itemize}
    \item Single optimization run showed 83.0\% accuracy vs. 80.8\% baseline (+2.2\% gain)
    \item 70.8\% feature reduction (2048→598) with apparent improvement
    \item Completed in 3.6 hours on 2K subset
\end{itemize}

\textbf{What Statistical Validation Revealed (30-Run Reality):}
\begin{itemize}
    \item $\times$ \textbf{Optimization Failed:} DIO-XGBoost 81.91\% ± 1.38\% \textbf{significantly worse} than defaults 83.27\% ± 1.25\% (p=7.15×10\textsuperscript{-5}, ***)
    \item $\times$ \textbf{Insufficient Budget:} 576 evaluations inadequate for 2051-D search space (need 10,000-50,000)
    \item $\times$ \textbf{Feature Selection Overfitted:} 70.8\% reduction removed informative features
    \item $\times$ \textbf{Single-Split Misleading:} Initial result was optimization overfitting to one partition
\end{itemize}

\textbf{Critical Lessons Learned:}
\begin{enumerate}
    \item \textbf{Budget Must Scale with Dimensionality:} 30-D medical (10K evals) → SUCCESS | 2048-D vision (576 evals) → FAILURE
    \item \textbf{Single Runs Are Deceptive:} Always validate with 30+ independent runs
    \item \textbf{No Algorithm Immune to Under-Resourcing:} Even XGBoost's regularization cannot overcome severe budget constraints
    \item \textbf{Cross-Domain Success Requires Appropriate Resources:} Transfer learning framework is sound, but computational investment must match problem complexity
\end{enumerate}

This honest negative result strengthens the research by demonstrating: (1) methodological rigor through statistical validation, (2) clear boundaries of when DIO succeeds vs. fails, and (3) practical guidance for practitioners on budget allocation. The contrast between medical success and vision failure provides valuable insights into optimization budget scaling laws.
...
This study demonstrated both the effectiveness and the limitations of the Dholes-Inspired Optimization algorithm for simultaneous feature selection and hyperparameter optimization across diverse machine learning domains. By developing a complete Python-based implementation of DIO and designing a novel nested optimization framework, we achieved robust and efficient models for medical classification (breast cancer diagnosis, 96.88\% accuracy with 67\% feature reduction), while discovering critical failure modes in computer vision (CIFAR-10) that provide valuable insights into optimization budget requirements for high-dimensional problems.
...
\begin{enumerate}
    ...
    \item \textbf{CV-Based Optimization Methodology:} Demonstrated the critical importance of k-fold cross-validation within the optimization loop, preventing optimization overfitting and achieving 2.18\% accuracy improvement over single-split optimization (94.37\% → 96.55\%).
    ...
    \item \textbf{Pareto-Optimal Solutions:} Identified distinct Pareto-optimal solutions for medical domain (RF-CV: 96.55\%/6 features vs XGBoost: 96.88\%/10 features), representing different accuracy-complexity trade-offs and enabling deployment flexibility based on application priorities.
    \item \textbf{High-Dimensional Failure Analysis:} Documented CIFAR-10 optimization failure (81.91\% vs 83.27\% defaults, p<0.0001) on 2048-D features, revealing that budget must scale with dimensionality\textsuperscript{2}—a critical lesson for practitioners.
...
\textbf{1. RF Single-Split Optimization:} Achieved 94.37\% $\pm$ 1.82\% with 4 features (87\% reduction), ranking \#6. Revealed "optimization overfitting" where hyperparameters tuned on single partition achieved 99\% on that split but underperformed defaults across 30 runs, demonstrating the critical need for cross-validation during optimization.
...
\textbf{Computer Vision Domain (CIFAR-10 Classification) - Failure Case:}

\textbf{Single-Run Illusion:} Initial DIO-XGBoost result appeared promising: 83.0\% accuracy on optimization subset (2K train, 500 test) vs. 80.8\% baseline (+2.2\%), with 598/2048 features (70.8\% reduction). Optimization completed in 3.6 hours (215.98 minutes).

\textbf{Statistical Validation Revealed Failure:} 30-run validation on the same dataset demonstrated the single-run result was \textbf{optimization overfitting}. DIO-XGBoost achieved only 81.91\% $\pm$ 1.38\% across 30 runs—\textbf{significantly worse} than default XGBoost (83.27\% $\pm$ 1.25\%, p=7.15×10\textsuperscript{-5}, ***). Root cause: 576 total function evaluations proved catastrophically insufficient for a 2051-dimensional search space (2048 features + 3 hyperparameters); proper optimization would require 10,000-50,000 evaluations. This failure demonstrates that budget constraints, not the DIO algorithm itself, determine success in high-dimensional problems.

\textbf{Cross-Domain Methodological Insights:}
\begin{enumerate}
    \item \textbf{Medical Success vs Vision Failure:} DIO achieved +2.18\% accuracy gain in 30-D medical data (94.37\% → 96.55\% with CV), but failed catastrophically in 2048-D vision data (-1.36\% relative to defaults, p<0.0001). The difference: medical domain had adequate budget (10K evals for 34-D space), vision domain was severely under-resourced (576 evals for 2051-D space).
    
    \item \textbf{Optimization Overfitting is Algorithm-Dependent:} RF single-split suffered from overfitting, while XGBoost single-split achieved top performance on medical data, validating gradient boosting's robustness for low-dimensional problems.
    
    \item \textbf{Budget-Dependent Success:} Even in pre-compressed ResNet50 representations, DIO eliminated 70.8\% of features, but insufficient budget (576 vs needed 10,000+ evaluations) caused statistical underperformance.
    
    \item \textbf{Computational Scaling:} Fast DIO configurations (3 dholes, 8 iterations) proved inadequate for 68$\times$ larger feature spaces---budget must scale with dimensionality.
    
    \item \textbf{Model Selection Criticality:} Systematic pre-optimization comparison identified optimal candidates (RF-CV for medical 96.55\%, XGBoost for images 85\%), avoiding wasted computational resources.
\end{enumerate}

% --- Schema 3: Cross-Domain Results Comparison Table ---
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth,height=0.65\textheight,keepaspectratio]{schemas and snippets/schema3_cross_domain_results_table.png}
    \caption{
        \textbf{Cross-Domain Validation Results:} Quantitative comparison showing Medical SUCCESS (30D, 96.88\% ± 1.10\%, rank \#1, p=0.0047, 10 features, 54 sec) vs CIFAR-10 FAILURE (2048D, 81.91\% ± 1.38\%, rank \#3 worse than 83.27\% defaults, p<0.0001, 598 features, 215.98 min/3.6 hrs). Critical insight: 30-D with 10,000 evals → SUCCESS; 2048-D with 576 evals → FAILURE. Optimization budget must scale with dimensionality², requiring 17-87× more evaluations for high-dimensional problems. Validates that computational investment determines cross-domain success.
    }
...
\begin{enumerate}
    ...
    \item \textbf{Domain-Dependent Effectiveness:} Results demonstrate DIO's robustness is conditional, not universal—significant improvements on low-dimensional tabular medical data (30-D) versus clear failure on under-budgeted high-dimensional vision features (2048-D), establishing critical applicability boundaries.
\end{enumerate}
