...
Our research led to an unexpected discovery we call \textbf{optimization overfitting}. When we optimized Random Forest on a single train/test split, we achieved perfect training accuracy but the configuration generalized poorly—averaging just 94.72\% across 30 different data partitions, which was actually worse than using default parameters (94.89\%). This taught us that algorithm choice matters enormously. When we switched to cross-validation during optimization, Random Forest performance jumped to 96.26\% (p$<$0.001). More surprisingly, when we tried XGBoost instead, it didn't suffer from this problem at all on medical data—achieving 96.34\% $\pm$ 1.23\% (our best result) even without cross-validation, significantly beating defaults (p=0.0426). However, when we moved to high-dimensional image data (2048 features), we discovered that even XGBoost couldn't overcome an insufficient optimization budget. With only 576 function evaluations for a 2051-dimensional search space, the optimized model (81.91\% $\pm$ 1.38\%) actually performed worse than defaults (83.27\% $\pm$ 1.25\%, p$<$0.0001).

For breast cancer classification, our best configuration—DIO-optimized XGBoost with 17 features—ranked first among all methods we tested, demonstrating that proper optimization can extract excellent performance while reducing feature count by 43\%. The RF approach with cross-validation achieved nearly identical accuracy (96.26\%) using just 6 features, offering an 80\% reduction for resource-constrained settings. Statistical validation with Wilcoxon tests confirmed these improvements were genuine, not artifacts of random variation.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        Baseline Accuracy & 94.72\% $\pm$ 2.35\% & 83.27\% $\pm$ 1.25\% & Domain complexity \\
        DIO Accuracy & 96.26\% $\pm$ 1.33\% & 81.91\% $\pm$ 1.38\% & Success vs. Overfitting \\
...
\begin{enumerate}
    ...
    \item \textbf{At Least Model Selection Worked:} Despite the optimization struggles, our initial model comparison was validated: XGBoost really is the best choice for CIFAR-10 features (83.27\% default), and RF-CV really is best for medical data (96.26\% optimized). So the methodology for \emph{choosing} which algorithm to optimize worked correctly, even if fully optimizing it proved harder than expected.
\end{enumerate}
...
On the medical data, everything clicked into place. DIO-optimized XGBoost hit 96.34\% accuracy with just 17 features—our best result overall, and statistically better than defaults (p=0.0426). The RF approach with cross-validation came in close behind at 96.26\% using only 6 features. These aren't just good numbers; they represent deployable clinical models that could actually reduce diagnostic costs while maintaining accuracy.
