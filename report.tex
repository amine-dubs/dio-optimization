...
\begin{itemize}
    ...
    \begin{equation}
        V = 2 - t \times \left( \frac{2}{\text{Max\_iter}} \right)
    \end{equation}
    where $t$ is the current iteration and $\text{Max\_iter}$ is the maximum number of iterations.
    ...
    \item \textbf{Oscillation Coefficient (Eq. 3):} This coefficient adds a controlled stochastic element to the movement.
    \begin{equation}
        C = r + \sin(r \times \pi)
    \end{equation}

    \item \textbf{Adjusted Distance to Leader (Eq. 4):} Calculates the adjusted distance to the leader.
    \begin{equation}
        \vec{D}_{\text{lead}} = |C \times \vec{\text{lead\_vocalizer\_pos}}^2 - \vec{D}_i^2|
    \end{equation}
    where $\vec{\text{lead\_vocalizer\_pos}}$ is the position of the lead dhole and $\vec{D}_i$ is the position of the current dhole.

    \item \textbf{New Position Update (Eq. 5):} The new position of a dhole is calculated based on the leader's position and the adjusted distance.
    \begin{equation}
        \vec{X}_{\text{lead}} = \vec{\text{lead\_vocalizer\_pos}} + B \times \sqrt{\vec{D}_{\text{lead}}}
    \end{equation}
\end{itemize}
...
\begin{lstlisting}[language=Python, caption={DIO Algorithm Pseudocode}, label={lst:dio_pseudocode}]
Algorithm DIO()
1. Initialize population of dholes D randomly
2. Evaluate fitness of each dhole
3. Set lead_vocalizer = best dhole
4. while t < Max_iter do
5.     Update vocalization influence V (Eq. 1)
6.     for i = 1 to N do
7.         Calculate B = V * r (Eq. 2)
8.         Calculate C = r + sin(r * pi) (Eq. 3)
9.         Calculate D_lead = |C * lead_pos^2 - D_i^2| (Eq. 4)
10.        Update position X_lead = lead_pos + B * sqrt(D_lead) (Eq. 5)
11.        Apply boundary constraints (Eq. 6)
12.        Evaluate new fitness
13.        if fitness(D_i) < fitness(lead): update leader (Eq. 7)
14.    end for
15.    t = t + 1
16. end while
17. Return lead_vocalizer as best solution
\end{lstlisting}
