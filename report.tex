...
\begin{figure}[H]
    ...
    \caption{Comprehensive 6-panel comparison of all 10 models across 30 runs for single-split optimization approach.}
    \label{fig:main_viz}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Comprehensive 6-panel comparison of XGBoost-optimized model across 30 runs, showing superior performance and stability.}
    \label{fig:xgb_viz}
\end{figure}
...
\begin{enumerate}
    \item \textbf{Algorithm-Dependent Feature Requirements:} XGBoost requires more features (17) than RF (6-8) to achieve optimal performance, likely due to its sequential boosting nature requiring richer feature interactions.
    
    \item \textbf{Best Overall Accuracy:} XGBoost optimization achieved the highest accuracy (96.34\%) across all experiments, validating DIO's generalizability to gradient boosting algorithms.
    
    \item \textbf{Computational Efficiency:} Single-split XGBoost optimization (54 sec) was 526$\times$ faster than CV-based RF (7.9 hours) while achieving higher accuracy, suggesting XGBoost's inherent regularization reduces optimization overfitting risk.
    
    \item \textbf{Accuracy-Feature Trade-off:} RF with CV offers better feature compactness (6 features, 96.26\%) for maximum interpretability, while XGBoost offers slightly higher accuracy (96.34\%) with moderate feature reduction (17 features).
\end{enumerate}
...
\textbf{Choose CV-RF (6 features, 96.26\%)} if:
\begin{itemize}
    \item Maximum interpretability is critical (6 clinically meaningful features)
    \item Cost minimization is priority (80\% fewer measurements)
    \item Simple linear decision boundaries suffice
    \item Computational training budget allows 7.9 hours
\end{itemize}

\textbf{Choose DIO-XGBoost (17 features, 96.34\%)} if:
\begin{itemize}
    \item Maximum accuracy is priority (highest observed: 96.34\%)
    \item Fast optimization is required (54 seconds vs. hours)
    \item Moderate feature reduction acceptable (43\% reduction)
    \item Complex feature interactions benefit diagnosis
\end{itemize}
...
\textbf{1. DIO-XGBoost-Optimized (17 features, 96.34\% $\pm$ 1.23\%):}
\begin{itemize}
    \item \textbf{Best Accuracy:} Highest performance across all experiments (\#1 rank)
    \item \textbf{Fast Optimization:} 54 seconds, practical for rapid prototyping
    \item \textbf{Moderate Feature Reduction:} 43.3\% reduction (17 features), balancing accuracy and efficiency
    \item \textbf{Lowest Variance:} 1.23\% std, most consistent across data partitions
    \item \textbf{Use Case:} High-stakes diagnosis where maximum accuracy justifies moderate complexity
\end{itemize}

\textbf{2. DIO-CV-RF (6 features, 96.26\% $\pm$ 1.33\%):}
\begin{itemize}
    \item \textbf{Maximum Interpretability:} Only 6 clinically meaningful features
    \item \textbf{Best Feature Compactness:} 80\% reduction, 5$\times$ faster inference
    \item \textbf{Cost Optimal:} 80\% reduction in laboratory measurements and associated costs
    \item \textbf{CV-Validated:} Hyperparameters guaranteed to generalize across populations
    \item \textbf{Use Case:} Resource-constrained settings, point-of-care testing, maximum interpretability required
\end{itemize}

\textbf{3. DIO-RF Single-Split (8 features, 94.72\% $\pm$ 1.41\%):}
\begin{itemize}
    \item \textbf{Ultra-Fast Optimization:} 1 minute, suitable for rapid iteration
    \item \textbf{Good Feature Reduction:} 73\% reduction (8 features)
    \item \textbf{Acceptable Accuracy:} 94.72\%, sufficient for many applications
    \item \textbf{Use Case:} Prototyping, research, non-critical screening applications
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Multi-Algorithm Validation:} Successfully applied DIO to both Random Forest and XGBoost, demonstrating framework generalizability across different classifier families (bagging vs. boosting).
    ...
    \item \textbf{Statistical Rigor:} Comprehensive validation through 30 independent runs with different train/test splits across three optimization approaches (RF single-split, RF CV-based, XGBoost single-split), ensuring robust statistical conclusions.
    \item \textbf{Multiple Pareto-Optimal Solutions:} Identified three distinct Pareto-optimal solutions representing different accuracy-complexity trade-offs, enabling deployment flexibility based on clinical priorities.
...
\textbf{1. RF Single-Split Optimization (Initial):} Achieved 94.72\% $\pm$ 1.41\% with 8 features (73\% reduction), ranking \#7. However, hyperparameters optimized on a single data partition (random\_state=42) achieved 100\% on that split but underperformed defaults across 30 runs (p=0.165), revealing "optimization overfitting."

\textbf{2. RF CV-Based Optimization (Improved):} Achieved \textbf{96.26\% $\pm$ 1.33\%} with \textbf{6 features (80\% reduction)}, ranking \textbf{\#3 overall}. By using 5-fold cross-validation during fitness evaluation, the optimized hyperparameters now \textit{significantly outperform} defaults (p=0.0084), demonstrating proper generalization. This represents the best feature compactness achieved.

\textbf{3. XGBoost Single-Split Optimization (Best Accuracy):} Achieved \textbf{96.34\% $\pm$ 1.23\%} with \textbf{17 features (43\% reduction)}, ranking \textbf{\#1 overall}—the highest accuracy across all experiments. Optimization completed in only 54 seconds, demonstrating that gradient boosting's inherent regularization may reduce optimization overfitting risk, making single-split viable for stable algorithms.

\textbf{Critical Methodological Insights:}
\begin{enumerate}
    \item \textbf{Optimization Overfitting is Algorithm-Dependent:} RF single-split suffered from optimization overfitting, while XGBoost single-split achieved top performance, suggesting gradient boosting's inherent regularization provides natural protection.
    
    \item \textbf{Feature Selection Proves Robust:} Across all approaches, feature selection (43-80\% reduction) consistently provided the primary value, with hyperparameter tuning adding measurable but secondary improvements.
    
    \item \textbf{Accuracy-Interpretability Trade-off:} XGBoost offers maximum accuracy (96.34\%, 17 features), RF-CV offers maximum interpretability (96.26\%, 6 features), representing different Pareto-optimal points.
    
    \item \textbf{CV Improves Both Accuracy and Stability:} RF-CV achieved both higher accuracy (96.26\% vs 94.72\%) and lower variance (1.33\% vs 1.41\%) than RF single-split, validating proper methodology.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Rapid Prototyping:} DIO-RF-Single (94.72\%, 8 features) for research and development
