"""
Statistical Model Comparison with CV-Optimized Configuration
=============================================================
Compares DIO-CV-optimized RF with baseline models using 30 independent runs
for statistical significance. Uses the CV-optimized hyperparameters from
the k-fold cross-validation optimization.
"""
...
print("="*80)
print("STATISTICAL MODEL COMPARISON - CV-OPTIMIZED CONFIGURATION")
print("="*80)
...
print(f"\n✓ Loaded CV-optimized configuration:")
print(f"  - Optimization method: {opt_config['method']}")
print(f"  - k-folds used: {opt_config['k_folds']}")
print(f"  - Selected features: {len(selected_features)}/30 ({opt_config['selected_features']['reduction_percentage']}% reduction)")
print(f"  - Feature indices: {selected_features}")
print(f"  - Feature names: {opt_config['selected_features']['names']}")
...
for model_name, metrics in all_results.items():
    summary = {
        ...
        'Features': len(selected_features) if 'Selected' in model_name or 'CV' in model_name else 30
    }
...
print("\nWilcoxon Signed-Rank Test (vs DIO-CV-Optimized RF):")
print("─" * 80)

dio_accuracy = all_results['DIO-CV-Optimized RF']['accuracy']
statistical_tests = []

for model_name in all_results.keys():
    if model_name != 'DIO-CV-Optimized RF':
...
print(f"\n{'='*80}")
print("SUMMARY STATISTICS (30 RUNS - CV-OPTIMIZED)")
print(f"{'='*80}")
...
fig.suptitle('Statistical Model Comparison - CV-Optimized (30 Independent Runs)', 
...
for model in stats_df['Model'][:6]:  # Top 6 models
    model_short = model.replace('DIO-CV-Optimized RF', 'DIO-CV').replace(' (CV-Selected)', '(CV-S)').replace(' (All)', '(A)')[:20]
    model_names_short.append(model_short)
    
# Create p-value matrix
dio_acc = all_results['DIO-CV-Optimized RF']['accuracy']
p_matrix = []
...
ax4.set_xticklabels(model_names_short, rotation=45, ha='right', fontsize=8)
...
plt.tight_layout()
plt.savefig('statistical_comparison_visualization_cv.png', dpi=300, bbox_inches='tight')
print("✓ Visualization saved to 'statistical_comparison_visualization_cv.png'")

# ========== ADDITIONAL PUBLICATION-QUALITY PLOTS ==========

# Plot 2: Individual Model Performance Trends
fig2, axes = plt.subplots(2, 5, figsize=(20, 8))
fig2.suptitle('Individual Model Performance Across 30 Runs (CV-Optimized)', fontsize=16, fontweight='bold')
...
plt.tight_layout()
plt.savefig('individual_model_trends_cv.png', dpi=300, bbox_inches='tight')
print("✓ Individual trends saved to 'individual_model_trends_cv.png'")

plt.show()

# ========== FINAL SUMMARY ==========
print(f"\n{'='*80}")
print("STATISTICAL COMPARISON COMPLETE - CV-OPTIMIZED")
print(f"{'='*80}")
print(f"\n📊 Summary:")
print(f"  • Optimization method: {opt_config['method']}")
print(f"  • k-folds used in optimization: {opt_config['k_folds']}")
...
print(f"\n✨ CV-Optimized Model Performance:")
dio_cv_stats = stats_df[stats_df['Model'] == 'DIO-CV-Optimized RF'].iloc[0]
print(f"  • Rank: #{dio_cv_stats['Rank']}")
print(f"  • Mean Accuracy: {dio_cv_stats['Mean_Accuracy']:.4f} ± {dio_cv_stats['Std_Accuracy']:.4f}")
print(f"  • Features used: {dio_cv_stats['Features']}/30 ({opt_config['selected_features']['reduction_percentage']}% reduction)")
print(f"  • Mean training time: {dio_cv_stats['Mean_Time']:.4f} seconds")
print(f"\n📁 Files generated:")
print(f"  1. statistical_comparison_summary_cv.csv")
print(f"  2. statistical_significance_tests_cv.csv")
print(f"  3. all_runs_detailed_results_cv.csv")
print(f"  4. statistical_comparison_visualization_cv.png")
print(f"  5. individual_model_trends_cv.png")
print(f"\n{'='*80}")
